...
- Enforce patched versions of security-sensitive dependencies installed into
  ...
  - `ip-address` npm package enforced at `>=10.3.1` (CVE-2026-69192).
    `tar`, `brace-expansion` and `ip-address` are installed globally with the
    patched versions before installing the JS tooling (e.g. `less`, `rtlcss`)
    so the patched versions take precedence.
...
- `odood deploy`: when `--py-version` is not given and the system Python is
  only "in range" for the target Odoo serie, Odood now builds the pinned patch
  release from `suggestPythonVersion` (e.g. `3.12.14` for Odoo 18/19) from
  source instead of reusing the distro's Python, unless the two already match.
  This makes deployments (and the prebuilt Docker images) run a reproducible
  interpreter; the Docker image no longer hardcodes `--py-version`.
